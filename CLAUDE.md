...
- אין JavaScript מעבר להכרחי: מתג השפה הוא קישור `<a>` רגיל, ואקורדיון ה"קרא עוד" (בכרטיסי "ייעוץ" ו"מודיעין תחרותי") בנוי על `<details>/<summary>` סמנטי ללא JS. קובץ ה-redirect העליון (`docs/index.html`) הוא היוצא מן הכלל היחיד - הוא משתמש ב-JS מינימלי (`location.replace`) כגיבוי ל-meta refresh.
